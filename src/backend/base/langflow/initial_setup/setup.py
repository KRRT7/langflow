...
def update_edges_with_latest_component_versions(project_data):
    ...
    # Use shallow copy for nodes and edges; deep copy node/edge dicts individually to avoid unnecessary recursion
    nodes = project_data.get("nodes", [])
    edges = project_data.get("edges", [])

    # Build node id map and node type map for fast access
    node_id_map = {node.get("id"): node for node in nodes}
    ...
    # Deepcopy (in-place) edges so changed data doesn't affect the original
    edges_copy = [deepcopy(edge) for edge in edges]
    # Deepcopy nodes as well for any later change
    nodes_copy = [deepcopy(node) for node in nodes]
    project_data_copy = {
        **project_data,
        "nodes": nodes_copy,
        "edges": edges_copy,
    ...
    # Build updated id maps for copied nodes
    copied_node_id_map = {node.get("id"): node for node in nodes_copy}
    ...
    for edge in edges_copy:
        ...
        source_node = copied_node_id_map.get(edge.get("source"))
        target_node = copied_node_id_map.get(edge.get("target"))

        # Try to reconcile missing source node by type
        if source_node is None and source_handle and "dataType" in source_handle:
            node_type = source_handle.get("dataType")
            type_ids = node_type_map.get(node_type)
            if type_ids:
                new_node_id = type_ids[0]
                ...
                source_node = copied_node_id_map.get(new_node_id)

                old_id_prefix = edge.get("id", "").split("{")[0]
                if old_id_prefix:
                    new_id_prefix = old_id_prefix.replace(str(edge.get("source")), str(new_node_id))
                    edge["id"] = edge.get("id", "").replace(old_id_prefix, new_id_prefix)

        # Try to reconcile missing target node by type in handle id
        if target_node is None and target_handle and "id" in target_handle:
            ...
            if id_parts:
                ...
                if type_ids:
                    ...
                    target_node = copied_node_id_map.get(new_node_id)

                    old_id_suffix = edge.get("id", "").split("}-")[1] if "}-" in edge.get("id", "") else ""
                    if old_id_suffix:
                        new_id_suffix = old_id_suffix.replace(str(edge.get("target")), str(new_node_id))
        ...
        if source_node and target_node:
            ...
            # Fast output lookups by name/display_name using dicts
            outputs = source_node_data.get("outputs", [])
            outputs_by_name = {o.get("name"): o for o in outputs}
            outputs_by_display = {o.get("display_name"): o for o in outputs}

            output_data = outputs_by_name.get(source_handle.get("name"))
            if not output_data:
                output_data = outputs_by_display.get(source_handle.get("name"))
                if output_data:
            ...
            template_field = template.get(field_name, {}) if field_name in template else None
            input_types = template_field.get("input_types", []) if template_field else []
            if field_name in template and target_handle.get("inputTypes", []) != input_types:
                edge_changes_log[target_node_data.get("display_name", "unknown")].append(
                    {
                        "attr": "inputTypes",
                        "old_value": target_handle.get("inputTypes", []),
                        "new_value": input_types,
                    }
                )
                target_handle["inputTypes"] = input_types
            ...
            old_escape_source_handle = escape_json_dump(safe_json_loads(edge.get("sourceHandle", "{}")))
            old_escape_target_handle = escape_json_dump(safe_json_loads(edge.get("targetHandle", "{}")))
